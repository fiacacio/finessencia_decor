...
export default async function Page() {
  ...
  let sectionColors: SectionColors = {}, sectionContent: SectionContent = {}
  if (isSupabaseConfigured) {
    try { const settings = await getSectionSettings(); sectionColors = settings.colors; sectionContent = settings.content } catch { /* Keep the original design when settings are unavailable. */ }
  }
  ...
  if (isSupabaseConfigured) {
    try { customerPopup = await getCustomerPopup() } catch { /* The public page remains available until the customer migration runs. */ }
  }
