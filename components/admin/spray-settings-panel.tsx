...
export function SpraySettingsPanel() {
  ...
  return <form className="admin-announcement" onSubmit={save} style={{ marginBottom: 24 }}>
    ...
    <fieldset disabled={!ready || saving}>
      ...
      <div className="admin-effect-timing"><strong>Movimento de ACENDA O MOMENTO</strong><p>O brilho percorre o texto da esquerda para a direita no desktop, sincronizado ao vídeo.</p><label>Primeira passagem: {settings.hero_first_effect_time.toFixed(2)} s<input type="number" min="0" max="59.99" step="0.01" value={settings.hero_first_effect_time} onChange={(e) => { setSettings({ ...settings, hero_first_effect_time: Number(e.target.value) }); setMessage('') }} /></label><label>Segunda passagem: {settings.hero_second_effect_time.toFixed(2)} s<input type="number" min="0.01" max="60" step="0.01" value={settings.hero_second_effect_time} onChange={(e) => { setSettings({ ...settings, hero_second_effect_time: Number(e.target.value) }); setMessage('') }} /></label></div>
      <label>Ampliação inicial de BEM-VINDA À FINESSÊNCIA: {settings.welcome_title_scale}%<input type="range" min="0" max="100" value={settings.welcome_title_scale} onChange={(e) => { setSettings({ ...settings, welcome_title_scale: Number(e.target.value) }); setMessage('') }} /></label>
